...
export async function uploadFileToStorage({ buffer, storedName, mimeType, originalName }) {
  ...
  if (env.STORAGE_PROVIDER === 's3' && !s3Active) {
    throw new Error('[Storage Error] STORAGE_PROVIDER is explicitly set to "s3" but AWS S3 credentials (AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_S3_BUCKET_NAME) are missing or invalid.');
  }
...
export async function getFileFromStorage(fileRecord) {
  if (fileRecord.storageProvider === 's3') {
    if (!isS3Configured()) {
      throw new Error('[Storage Error] File record is stored on AWS S3 but AWS credentials are not configured on this server.');
    }
    const s3Client = getS3Client();
    if (!s3Client) {
      throw new Error('[Storage Error] Failed to initialize S3 client for S3 storage fetch.');
    }
  ...
  if (!fs.existsSync(filePath)) {
    throw new Error(`[Storage Error] File "${fileRecord.storedName}" not found on local server storage path: ${filePath}`);
  }
