...
export default function Custom404ComingSoon() {
  ...
  // Clean path format for title (e.g. /workflows -> Workflows)
  const rawRouteName = attemptedPath
    ? attemptedPath.split('?')[0].replace(/^\/+/, '').replace(/-/g, ' ')
    : '';

  const formattedName = rawRouteName
    ? rawRouteName.charAt(0).toUpperCase() + rawRouteName.slice(1)
    : 'Requested Feature';
  ...
  return (
    <>
      <Head>
        <title>{formattedName} Coming Soon | NEURON_FLOW</title>
        <meta name="description" content="This page is currently under development on the NEURON_FLOW platform roadmap." />
      </Head>
        ...
        <section className="relative overflow-hidden bg-gradient-to-b from-[#141417] via-[#0d0d0f] to-[#09090b] px-6 py-20 lg:py-24 border-b border-[#27272a] text-center">
          ...
          <div className="max-w-4xl mx-auto relative z-10">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#ff4f00]/10 border border-[#ff4f00]/30 text-sm font-semibold text-[#ff4f00] mb-6">
              ...
              Route ({attemptedPath || '/coming-soon'}) Under Active Integration
            </div>

            <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-white mb-6">
              {formattedName} is <span className="text-[#ff4f00]">Coming Soon</span>
            </h1>

            <p className="text-base sm:text-lg text-[#a1a1aa] max-w-2xl mx-auto mb-10 leading-relaxed font-normal">
              You navigated to a page route currently being built for NEURON_FLOW 2.5. Our team is wiring up full API controllers, database schemas, and responsive UI components.
            </p>
            ...
            <div className="max-w-md mx-auto mb-12 bg-[#141417] p-4 rounded-xl border border-[#27272a] shadow-lg">
              {subscribed ? (
                <div className="p-4 rounded-lg bg-emerald-950/60 border border-emerald-500/30 text-emerald-400 text-sm font-medium flex items-center justify-center gap-2">
                  <span>✨</span> You are registered for early feature release notifications!
                </div>
              ) : (
        ...
        <section className="max-w-6xl mx-auto px-6 py-16 w-full flex-1">
          <div className="text-center mb-12">
            ...
            <h2 className="font-display text-3xl font-bold text-white mb-3">
              Explore Active Roadmap Modules
            </h2>
            <p className="text-sm text-[#a1a1aa] max-w-xl mx-auto">
              Check out the completion progress for all core modules currently being integrated into the platform.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {roadmapItems.map((item) => {
              const isMatch = attemptedPath.toLowerCase().includes(item.route.toLowerCase());
          ...
          <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              ...
              <p className="text-sm text-[#a1a1aa] mt-1 font-normal">
                © 2026 NEURON_FLOW. Synchronized dark theme overview dashboard.
              </p>
            </div>
